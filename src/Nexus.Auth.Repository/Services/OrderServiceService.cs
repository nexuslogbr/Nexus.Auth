...
namespace Nexus.Auth.Repository.Services
{
    public class OrderServiceService : IOrderServiceService
    {
        ...
        public async Task<GenericCommandResult<List<OrderServiceResponseDto>>> GetByChassi(GetByChassi obj, string path)
        {
            var result = await _accessDataService.PostDataAsync<List<OrderServiceResponseDto>>(path, "api/v1/OrderService/GetByChassi", obj);
            if (result is not null)
        ...
        public async Task<GenericCommandResult<OrderServiceResponseDto>> PutServicesOfOrder(OrderServiceUpdateDto obj, string path)
        {
            var result = await _accessDataService.PostDataAsync<OrderServiceResponseDto>(path, "api/v1/OrderService/PutServiceOfOrder", obj);
            if (result is not null)
                return new GenericCommandResult<OrderServiceResponseDto>(true, "Success", result, StatusCodes.Status200OK);
